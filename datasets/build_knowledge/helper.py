...
def get_value_from_csr(sparse_matrix, row, col):
    start = sparse_matrix.indptr[row]
    end = sparse_matrix.indptr[row + 1]
    col_indices = sparse_matrix.indices[start:end]
    
    # 查找指定列索引
    if col in col_indices:
        idx = np.where(col_indices == col)[0][0]
        return sparse_matrix.data[start + idx]
    else:
        return 0
...
def find_matching_of_a_segment_given_sorted_val_corres_idx(
    ...
    if criteria == "threshold":
        ...
        matched_steps = sorted_indices[mask]
        matched_steps_score = sorted_values[mask]
        
    elif criteria == "threshold+topK":
        # From the ones with sim-score > threshold, pick the top K if existing.
        mask = sorted_values > threshold
        matched_steps = sorted_indices[mask][:topK]
        matched_steps_score = sorted_values[mask][:topK]
    
    elif criteria == "topK":
        # Pick the top K
        matched_steps = sorted_indices[:topK]
        matched_steps_score = sorted_values[:topK]
